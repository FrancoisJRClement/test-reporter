...
<details>
  <summary>googletest-json (Experimental)</summary>

Support for C/C++ test results in JSON format generated by GoogleTest library.

To generate JSON output from GoogleTest executables, use the '--gtest_output=json' options (https://github.com/google/googletest/blob/main/docs/advanced.md#generating-a-json-report).

In GNU Autotools Projects, dedicated JSON output may be generated for each GoogleTest executable by specifying a test environment in Makefile.am files:

```Makefile
TESTS_ENVIRONMENT = <path to configuration scripts>/gtestHarness.sh
```

With the following "gtestHarness.sh" bash script:
```bash
#!/usr/bin/env bash

#set -x

$1 --gtest_output=json:${1}.json
```
</details>
